...
def make_transforms(H: int, train: bool=False):
    aug = []
    if train:
        aug += [A.GaussNoise(var_limit=(0.0,10.0), p=0.15),
...
class LatexTokenizer:
    def __init__(self, spm_model, bos_id=1, eos_id=2, pad_id=3):
        self.sp = spm.SentencePieceProcessor(); self.sp.load(spm_model)
    ...
    @property
    def vocab_size(self):
        return self.sp.get_piece_size() + 1  # +1 for pad
...
def pad_batch(batch,pad_id=0):
    ...
    for i,y in enumerate(ys): Y[i,:len(y)]=y
